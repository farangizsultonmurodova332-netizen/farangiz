            ...
            if (call.agora_channel) {
                ...
                if (clientRef.current.connectionState === "DISCONNECTED") {
                    ...
                    try {
                        ...
                        // If we raced and are now connected, ignore
                        if (joinError.code === "INVALID_OPERATION" && clientRef.current.connectionState !== "DISCONNECTED") {
                            console.log("[CallContext] Join raced, but we are connected now. Ignoring error.");
                        } else {
                            throw joinError;
                        }
                    }
                } else {
                    console.log("[CallContext] Manual join: Client already connected or connecting (" + clientRef.current.connectionState + "), skipping join.");
                }
            }
    ...
    const handleIncomingCallSignal = useCallback((signal: CallSignal) => {
        ...
        if (signal.type === "call_offer" && user && signal.callee_id === user.id) {
            const incomingCall: Call = {
                ...
                // Do NOT use token from signal. It is likely the caller's token.
                // We will get our own token when we answer or restore.
                agora_token: undefined,
            };
            setState((prev) => ({
                ...prev,
                call: incomingCall,
